...
`scene-builder/water-mesh.ts` builds a single `THREE.Mesh` — `PlaneGeometry(tank.width, tank.depth, 16, 16)` rotated −π/2 about X, positioned at `(tank.width / 2, tank.height − 5 mm, tank.depth / 2)` — i.e. 5 mm below the interior rim. **This is the ONLY water surface**: the Stage 10 v1 static `waterTint` plane that `tank-mesh.ts` used to add 30 mm below the rim was retired (two stacked planes 25 mm apart read as a rendering bug); the authored `style.waterTint` now bakes into this surface's `uBaseColor` uniform at build time (sRGB hex → linear via `Color.set`; absent ⇒ the editorial pale-blue default, bit-identical to the old literal). The cache tag therefore includes the tint — a tint edit disposes + rebuilds like a tank resize. The vertex shader displaces Y by two stacked sine bands:
