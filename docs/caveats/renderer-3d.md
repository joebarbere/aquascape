...
`scene-builder/water-mesh.ts` builds a single `THREE.Mesh` — `PlaneGeometry(tank.width, tank.depth, 16, 16)` rotated −π/2 about X, positioned at `(tank.width / 2, tank.height − WATER_OFFSET_BELOW_RIM_MM, tank.depth / 2)` — i.e. **25 mm below the interior rim** (raised from the original 5 mm; a near-rim waterline read as "filled to the brim"). **`WATER_OFFSET_BELOW_RIM_MM` is exported and is the single waterline source of truth**: the host's `LivestockSimulationService` subtracts it from `tank.height` when building the ECS `tankAabb.maxY`, so depth bands, the kinematic clamp, bubble despawn, and surface food sprites all track the visible surface — never hard-code a second copy. **This is the ONLY water surface**: the Stage 10 v1 static `waterTint` plane that `tank-mesh.ts` used to add 30 mm below the rim was retired (two stacked planes 25 mm apart read as a rendering bug); the authored `style.waterTint` now bakes into this surface's `uBaseColor` uniform at build time (sRGB hex → linear via `Color.set`; absent ⇒ the editorial pale-blue default, bit-identical to the old literal). The cache tag therefore includes the tint — a tint edit disposes + rebuilds like a tank resize. The vertex shader displaces Y by two stacked sine bands:
...
Cached on the renderer (`this.waterMesh: WaterMeshHandle | null`, tagged by `WxHxD:waterTint`). Tank resize or tint edit disposes + rebuilds. RAF tick calls `waterMesh.updateTime(performance.now() / 1000)` every frame; the handle no-ops after dispose so a stale tick is safe. Same detach-before-`disposeNode` discipline as the livestock bundle — the cached mesh must be removed from `currentContent` before the rebuild walker disposes the tree, otherwise the shared geometry + material would be GPU-disposed every render.
