...
- **`tankAabb.maxY` is the WATERLINE, not the glass rim.** The host builds it from `effectiveWaterLevelMm(scene.tank)` (scene-model — the authored adjustable fill line or the default 25 mm-below-rim fill) so everything keyed off the AABB top — depth-band fractions, the kinematic clamp, bubble despawn (`maxY − BUBBLE_WATERLINE_INSET_MM`), surface food sprites — matches the water surface the renderer paints. Passing the raw rim height reintroduces fish/bubbles/food floating in the air gap.
...
BubbleFluidStepSystem     (fidelity pass — steps one Stam slice per air-stone: buoyancy + seeded turbulence puff → stepBubbleSlice; runs before BubbleLifetime so bubbles sample the current field)
BubbleLifetimeSystem      (F11.5 + fidelity pass — rise (velocityY, fluid-v modulated) + lateral advection via sampleBubbleFluid; despawn at waterline OR lifetime ≤ 0)
```
...
- **Bubble fluid coupling (bubble fluid fidelity pass — now ACTIVE):** the previously-unwired **BubbleStableFluids2D** Stam slice (`domain/fluid-sim`) is now the PRIMARY bubble drift, replacing the lightweight helix. `bubble-fluid.ts` owns one vertical 2D slice per registered air-stone source (slice row ← world Y spanning the tank height; slice column ← world X centred on the source). `bubbleFluidStepSystem` (runs after `bubbleSourceSpawnSystem`, before `bubbleLifetimeSystem`) injects steady upward **buoyancy** at the source column band + a **deterministic seeded turbulence puff** (`tickPrng(world, BUBBLE_FLUID_KEY, s, …)` — NEVER `Math.random`), then `stepBubbleSlice` once on the fixed sim dt. `bubbleLifetimeSystem` calls `sampleBubbleFluid(world, x, y, scratch)` — which **sums** the lateral `u` (X drift) + vertical `v` (rise modulation, clamped to ±35 % of `velocityY` so the net rise stays clearly upward) across ALL slices at the bubble's position. Because slices overlap in world space, two air-stones' plumes genuinely interact (the F11.5-reserved "multi-stone column interaction"). Z drift = `BUBBLE_FLUID_Z_FRACTION` (0.55) of the lateral magnitude with a per-bubble `spawnSeq` phase so a 2D slice still reads as a 3D, non-planar plume.
  - **Determinism:** `stepBubbleSlice` is pure of (slice state, dt, forces); the only entropy is the seeded puff; slices are stepped + sampled in fixed source-index order (float-add order-stable). The 1000-tick two-world replay (incl. `WorldSnapshot.bubblePosition`) stays byte-identical — covered by `bubble-fluid.spec.ts` (slice-field byte-equality) + the existing F11.5 Wave 5 case in `determinism.spec.ts` (now exercises the fluid path through `world.step()`).
  - **Perf:** slices are SMALL (`BUBBLE_FLUID_GRID = 16` → 18² cells) and only as many as there are air-stones (typically 1–2). The per-tick force write reuses a per-world scratch buffer (no hot-path allocation). The whole coupling early-outs when no source is registered. Measured p95 at n=200 + 2 air-stones ≈ 3.6 ms (this box) — inside the 4 ms budget.
  - **Slice lifecycle:** `registerBubbleSources` + `setTankAabb` rebuild the per-source slice set (`rebuildBubbleFluid`) — a fresh zero-velocity field is the deterministic starting point. Read-only diagnostics live on `world.__bubbleFluid` (a `BubbleFluidState`).
  - **Fallback:** the height-driven helix (`BUBBLE_WOBBLE_VEL_MM_PER_S` 28 / `BUBBLE_WOBBLE_WAVENUMBER` 0.045) is retained ONLY for tests that drive `bubbleLifetimeSystem` directly without ever registering sources (so no slices exist). The normal `world.step()` path always advects on the fluid field.
...
## Player-control seam (Stage 16 F16.1 — the ONE live-input boundary)

Game modes (`--mode game:<submode>`) let live keyboard/gamepad input drive a single fish. That input is the **one place** non-deterministic state enters the otherwise byte-identical world — keep it to this seam:

- **`world.setPlayer(eid)`** tags one fish with the `Player` component (replaces any prior player). **`world.clearPlayer()`** / `setPlayer(NO_ENTITY_REF)` relinquishes control. **`world.getPlayerEntity()`** returns the marked eid (or `NO_ENTITY_REF`). **`world.setPlayerVelocity(vx, vy, vz)`** stores a live velocity (mm/s, canonical doc coords).
- **Where the velocity is injected:** `world.step()` writes the stored player velocity onto the player's `Velocity` slab at the **very top of the tick, before any system runs**. `SteeringIntegrator` then **skips any `Player`-tagged entity** (early-`continue` after resetting its `Force`), so the AI behaviour forces (schooling/depth/flow/…) never overwrite the input. `KinematicSystem` integrates the injected velocity exactly like any other fish, and the post-step AABB clamp keeps the player inside the glass. Collision separation (fish-vs-fish) still applies — a player jammed into a school gets nudged, which is correct physics.
- **Determinism preserved for non-game worlds:** the injection branch is guarded on `playerEid !== NO_ENTITY_REF`. With no player marked (the default, and every editor/showcase world) the branch is skipped entirely and the tick is **bit-identical to a pre-F16.1 world** — the 1000-tick replay holds. Covered by `player-seam.spec.ts` (a no-player world replays byte-identically) on top of the existing `determinism.spec.ts`.
- **The player ENTITY is deterministic; only the INPUT is live.** Spawn the player the same way as any fish (same seed → same spawn position/orientation). Never read `Date.now()` / `Math.random()` / wall-clock inside the sim to drive the player — feed it only through `setPlayerVelocity`. The `Player` tag is the seam's only ECS-visible footprint (the renderer's fish-eye follow uses `getPlayerEntity()` to pick which snapshot index to ride); the live velocity lives on the world, not in a snapshot slab, so `WorldSnapshot`'s shape is unchanged. See `docs/caveats/game-modes.md`.
- **`world.setPlayerPredator(flag)` (F16.4 — predator game mode)** adds/removes the existing `Predator` tag on the marked player so the predator game REUSES `FearSystem`'s proximity path (`predatorProximityRisk`) — prey flee the player with zero new fear/flee code. No-op when no player is marked. `clearPlayer()` strips the tag from the departing player so a formerly-player fish doesn't keep scaring prey after the game ends. The tag is the only ECS footprint (no snapshot-shape change), so a NON-predator game or any non-game world is unaffected and the byte-identical replay holds.
- **The catch/despawn is NOT in the sim core.** The predator game's catch detection + `world.despawn(prey)` run in the app loop (`PredatorGameService.frame`), BETWEEN sim ticks — never inside `world.step()` / a system. A catch is gated on the LIVE player position, so it's a non-deterministic GAME EVENT kept out of the seeded tick stream (same class as the editor's out-of-tick add/remove livestock). Only an active game with a live player ever despawns prey → non-game worlds replay byte-identically. See `docs/caveats/game-modes.md` → "The catch/despawn determinism boundary".
