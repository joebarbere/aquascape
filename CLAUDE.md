...
**Stages 0 + 1 + 2 + 3 all complete (F3.1 – F3.5 shipped).** Both apps boot and round-trip a document end-to-end: pick a tank, style it, sculpt substrate, drag hardscape from the palette onto the canvas, click to select, mirror / duplicate / delete / reorder with the floating inspector + keyboard, save, reopen — identical. The full Stage 3 deliverable set: hardscape catalog kind (F3.5, 6 manifests bundled), real `hitTest` (F3.3, polygon-in-silhouette with viewport + object transform inversion), `selection` NgRx feature (F3.3), renderer paints hardscape silhouettes + selection handles (F3.3 + F3.5), `MirrorObject` + `ReorderObjectInLayer` commands (F3.3 + F3.4), `hardscape-tool` palette with pointer-events DnD (F3.1 + F3.2), canvas click-select + drop-receive in `apps/web` (F3.3), and the F3.4 floating selection inspector (Mirror H / Mirror V / Duplicate / Z up/down / Delete + Del/Cmd+D/`[`/`]` shortcuts) in `editor-shell`. The `.aqua` v1 format lives in `@aquascape/domain/document`; the catalog system in `@aquascape/domain/catalog` (now `substrate` + `hardscape`). **v1 is locked**. **Stage 3 follow-ups (deferred to a Stage 3.x patch):** actual drag-to-move / drag-to-rotate / drag-to-scale via the painted handles (handles render today but they're click targets only via the inspector; the painted dots aren't interactive yet), and marquee selection (the action + selector are wired but the canvas-side pointer-move-rectangle isn't drawn yet). Stage 4 (planting + growth simulation) is next.
