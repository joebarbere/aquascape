...
**Stages 0–6 complete, Stage 7 F7.1 landed.** Stage 6: F6.1 image export, F6.2 layout summary, F6.3 photo backdrop (view-only), F6.4 PWA install + service worker (offline) + electron-builder packaged installers (macOS DMG, Windows NSIS, Linux AppImage — unsigned). Stage 7 F7.1: livestock catalog (8 core species across fish / shrimp / snail with adult size + temperament + temp/pH ranges + schooling-min + bioload class + compatibility flags); `LivestockToolComponent` in `libs/features/livestock-equipment/` with a click-to-add browser + inventory list (quantity +/−/×) wired into the sidebar; `AddLivestockEntry` / `RemoveLivestockEntry` / `UpdateLivestockQuantity` commands; livestock promoted from envelope-only-passthrough to scene-resident state so commands participate in undo/redo. Both apps boot and round-trip a document end-to-end: pick a tank from 36 presets (ADA / UNS / Waterbox / US standard), style it, sculpt substrate, drag hardscape (19) + plants (28) from paginated palettes, click-drag bodies / scale handles / rotate dots / shift-marquee, mirror / duplicate / delete / reorder / group via floating inspector + keyboard, manage layers from the right rail, scrub the time slider to preview growth weeks 0–52, **flip golden-ratio / thirds / focal-point overlays on for precision framing**, **build a livestock inventory from the sidebar**, save, reopen — identical. UX polish: per-panel accordion collapse, resizable + collapsible side panes, responsive layout (< 768 px drawer overlays, 768–1199 px default-collapses rail, ≥ 1200 px full), geometric-A monogram favicon. The `.aqua` v1 format lives in `@aquascape/domain/document`; catalog in `@aquascape/domain/catalog` (`substrate` × 6 + `hardscape` × 19 + `plant` × 28 + `livestock` × 8). **v1 is locked** — every future format change needs a `Migration` entry. **Next:** Stage 7 F7.2 (stocking guidance — bioload + compatibility warnings) then F7.3 (equipment browser, mirroring livestock) then F7.4 (setup sheet — extend the F6.2 summary).
...
- **Marshaling preserves unknown extensions, with one deliberate asymmetry.** `documentToScene` / `sceneToDocument` carry the envelope (`meta` + optional `equipment` / `renderHistory` / `extensions`) verbatim — don't drop what you don't understand. **Livestock is the exception (Stage 7 F7.1):** it lives on `Scene.livestock` (not on `DocumentEnvelope`) so the Command pipeline can mutate it for undo/redo. The marshal moves it from `doc.livestock` to `scene.livestock` on load and back on save. F7.3 will repeat this promotion for equipment; until then equipment stays envelope-side. **DO NOT also expose livestock on `DocumentEnvelope`** — there's only one writer, and that's the Scene.
...
- **Storage key namespaces:** `aquascape.ui.collapsed.<panel>` (per-panel accordion — `tank-setup`, `substrate-tool`, `hardscape-tool`, `planting-tool`, `livestock-equipment`, `layers-panel`, `composition-overlays`, `snap-settings`, `wall-background`, `backdrop`), `aquascape.ui.shell.{sidebarWidth, railWidth, sidebarCollapsed, railCollapsed}` (layout), `aquascape.ui.overlays.{goldenRatio, thirds, focalPoints}` (F5.3 composition overlays — each flag persisted independently, defaults false), `aquascape.ui.wall.{enabled, color, widthMm, heightMm}` (Stage 5.x wall background — defaults off / `#2a2d35` / 1200 × 600 mm), `aquascape.ui.snap.{enabled, toGrid, toGuides, toObjects, gridSizeMm, toleranceCssPx}` (F5.4 snap — defaults all on / 10 mm / 8 CSS px), `aquascape.ui.backdrop.{enabled, dataUrl, opacity}` (F6.3 backdrop photo — data URL inlined for per-install persistence, cap `MAX_BACKDROP_BYTES = 8 MB`), `aquascape.templates.personal` (F5.2 personal templates — JSON array of `TemplateListing`, capped at `MAX_PERSONAL_TEMPLATES = 32`). `aquascape.autosaveDraft` for crash recovery.
...
- **Livestock is scene-resident, NOT a SceneObject.** `Scene.livestock?: LivestockEntry[]` is a flat planning-inventory array — entries DO NOT belong to a `Layer`, are NOT painted on the canvas, and are NOT subject to the lock-guard policy. The `decorObjectId?: Uuid` slot is reserved for a future "decorative fish swimming" feature that would link an entry to a `DecorObject` in a layer; F7.1 ships without that link.
- **Livestock UI is inventory, not placement.** `LivestockToolComponent` has NO `HostListener` on document pointer events and NO drag service — clicking a catalog tile dispatches `AddLivestockEntry` immediately. F7.1 deliberately doesn't compete with the canvas for pointer input. If F7.3's equipment-tool needs different mechanics, it can introduce its own service without breaking F7.1's pattern.
...
- **Icon pipeline lives in `tools/build-icons.mjs`.** Source of truth: `apps/web/src/favicon.svg`. Run `pnpm icons` to regenerate the full set: `apps/desktop/src/assets/icon.{png,ico,icns}` for Electron + `apps/web/src/icons/icon-{180,192,384,512}.png` for the PWA web manifest. Per-size rasterisation (`sharp` with density-boost) so glyph hinting stays sharp at every target size — not a blurry downscale from one master. ICNS: built via Apple's `iconutil` (macOS only; non-macOS warns + skips, trusting the committed binary). Production packaging (electron-builder, Stage 6 F6.4 follow-up) will embed the same ICNS in the `.app` bundle.
