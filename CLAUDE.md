...
- **Paint order:** background → grid → tank outline → substrate (Catmull-Rom silhouette + deterministic grain noise, `seed XOR fnv32(region.id)`) → water tint (inside tank, `globalAlpha = 0.25`) → frame overlay (8 mm rim for `'framed'`, +10 mm centre brace for `'braced'`) → hardscape silhouettes → plant silhouettes → selection handles (drawn LAST).
...
- **Storage key namespaces:** `aquascape.ui.collapsed.<panel>` (per-panel accordion), `aquascape.ui.shell.{sidebarWidth, railWidth, sidebarCollapsed, railCollapsed}` (layout). `aquascape.autosaveDraft` for crash recovery.
...
- **Electron icon pipeline is a follow-up** (ICO + ICNS conversions, `BrowserWindow({ icon: ... })` wiring). Web favicon at `apps/web/src/favicon.svg` ships now.
