...
- **Render() arg list is at its sensible ceiling.** `render(scene, viewport, catalog?, selection?, previewAgeWeeks?, overlayOptions?, wallBackground?, snapGuides?)` — 8 positional args. The next addition MUST refactor to an options object; the comment in `renderer-api/index.ts` flags this for the next contributor.
