...
**Stage 0 is complete** — the walking skeleton ships. Both apps boot: `apps/web` (Angular 18 standalone, ESBuild `application` builder) renders an empty tank + mm grid via `renderer-2d`; `apps/desktop` (hand-rolled Electron 33 with `electron-builder` for packaging, per ADR-0001) loads that same web bundle behind the non-negotiable security posture (contextIsolation, sandbox, no nodeIntegration, no remote, typed preload bridge, CSP enforced via HTTP header). The `domain/*`, `rendering/*`, and `platform/*` libs from plan §2.1 are implemented and tested (see "Stage 0 deliverables" below). Stage 1 picks up at `domain/document` (F1.3) and tank-setup UI (F1.1/F1.2).
...
## Stage 0 deliverables (what's actually in each lib)
...
- `libs/rendering/renderer-2d/` — `Canvas2DRenderer` drawing the tank rect + 10 mm/50 mm grid, DPR-aware, idempotent, listener-clean on dispose. Tests use a hand-rolled op-recording canvas (not jsdom canvas); a real-pixel snapshot is deliberately deferred to F6.1 where pixel correctness is the deliverable. `hitTest` returns `null` until F3.3.
...
- `apps/web/` — Angular 18 standalone bootstrap, `OnPush` everywhere, `ResizeObserver` driving redraw. Composition root in `src/main.ts` calls `selectPlatform()` (runtime-detects Electron via `window.aquascape.ipc`) and provides the four `platform-api` tokens.
...
**Lib build prerequisite (load-bearing):** every buildable lib needs a `package.json` with `"name": "@aquascape/..."` matching its tsconfig path alias, or `@nx/js:tsc` cross-lib builds break with `TS6059: not under 'rootDir'` for transitive consumers. This is now in place for every lib; remember it when scaffolding new ones in later stages.
...
The web shell renders an empty tank + mm grid via `@aquascape/rendering/renderer-2d`. `apps/web/src/select-platform.ts` runtime-detects the Electron preload bridge (`window.aquascape.ipc`) and binds the `platform-api` tokens to `platform-electron` under Electron, `platform-web` in the browser. Component / feature work in Stage 1+ lives in `libs/features/*` and is composed into the shell, not coded inline here.

The `lint` target enforces the `@nx/enforce-module-boundaries` rule defined in `eslint.config.cjs` — every layering violation from plan §2.2 fails CI at the `lint` step. Per-lib `coverageThreshold: 90%` is set in `libs/domain/*`, `libs/rendering/renderer-2d`, and the `libs/platform/platform-{web,electron}` `jest.config.ts` files. **Known gap:** the workspace uses the `@nx/jest/plugin` inference plugin for the `test` target, so `pnpm exec nx test <lib> --configuration=ci` does not currently pass `--coverage` — the thresholds are not yet fired by CI. Tracking as a Stage 0 follow-up; fix is either to switch back to the `@nx/jest:jest` executor or to configure the plugin's `ciTargetName` overrides. The `.github/workflows/pr.yml` `coverage` job runs `jest --coverage` explicitly for `tag:scope:domain`, so domain libs are gated; rendering and platform libs are not yet wired into that job.
...
- When changing the document format, change `aqua-document.ts` and `aqua-document.schema.json` **together**, re-validate `example.aqua.json` against the schema, and add a `Migration` entry — never modify v1 in place once Stage 1 has shipped.
