...
**Stages 0–7 + Stage 10 v1 complete; v2 of the `.aqua` format shipped.** Stage 10 v1: Three.js `renderer-3d` lib implementing the existing `SceneRenderer` interface (read-only / simulation-only — `hitTest` returns null in 3D, no selection or editing); `ViewModeService` + toolbar `ViewToggleComponent` (segmented 2D | 3D pill, Cmd/Ctrl+Shift+3 keyboard shortcut); `apps/web` hosts two stacked `<canvas>` elements (one per context type — a single canvas can only have one context for its lifetime) with a renderer-swap effect that disposes the prior renderer + attaches the now-active one on mode change. **Stage 10 v1.1:** schema v1 → v2 adds optional `Layer.zone` ('foreground' | 'midground' | 'background'); the 3D renderer reads it to band-remap object Z into front/mid/back thirds of the tank, clamps hardscape + plant positions so their AABB stays inside the glass, and applies deterministic per-vertex noise to hardscape so rocks read as irregular rather than flat-extruded slabs. Layers panel gains a per-row zone dropdown that dispatches the new `SetLayerZone` command. **Two new planned stages sit alongside the development plan**: [`plans/stage-11-animated-livestock.md`](plans/stage-11-animated-livestock.md) (Stage 11 — ECS-driven animated fish: schooling, territoriality, fear, feeding, flow-field drift, bubble columns, hardscape SDF collision; **plus** the ambient 3D polish originally floated as Stage 10 v2 — animated water surface, dynamic lighting / day-night cycle, plant sway — folded in as substage F11.7; anchored by the research bibliography at [`docs/research/stage-11-livestock-subsystem.md`](docs/research/stage-11-livestock-subsystem.md)) and [`plans/stage-12-release-pipeline.md`](plans/stage-12-release-pipeline.md) (Stage 12 — `pnpm release <version>`, electron-builder installers, GitHub-published releases; the script is scheme-agnostic — the actual version scheme is an open decision tracked inside the plan, ADR-0005 lands when the maintainer settles on it). **Next:** Stage 11 (animated livestock + ambient 3D polish), Stage 12 (release pipeline — cut the first release once a version scheme is chosen), or Stages 8 (community gallery) + 9 (AI render) if other priorities surface.
