...
**Stages 0 + 1 + 2 all complete (F2.1 – F2.4 shipped).** Both apps boot and round-trip a document end-to-end: pick a tank, style it, sculpt substrate (multi-region, per-region material from the catalog + Catmull-Rom height profile + deterministic seeded grain noise), save, reopen — identical. `apps/web` hosts the top toolbar (`features/editor-shell`: New / Open / Save / Save As / Recent + Ctrl/Cmd shortcuts + autosave-recovery banner), a tank-setup sidebar, and a substrate-tool sidebar (numeric editor — canvas drag handles deferred to F3.3). `apps/desktop` loads the same web bundle behind the non-negotiable security posture (contextIsolation, sandbox, no nodeIntegration, no remote, typed preload bridge, CSP) and routes file IO + native dialogs + key-value storage through a validated IPC contract. The `.aqua` v1 format lives in `@aquascape/domain/document` (AJV validator + `Migration` chain + ZIP container via `fflate`); the catalog system lives in `@aquascape/domain/catalog` (content-type-agnostic loader, per-entry JSON manifests, six bundled core substrates). **v1 is locked** — every future format change requires a `Migration` entry. Stage 3 (hardscape tool — drag-and-drop, transform handles, mirror/flip) is next.
...
## Stage 0 + Stage 1 + Stage 2 deliverables (what's actually in each lib)
