...
- **Play the tank** — `--mode game:<submode>` / `?mode=game:<submode>` drops you into a fish-eye, player-controlled mini-game (WASD/arrows to swim; gamepad support is planned). **All four sub-modes are fully playable** — each with a clear objective, win/lose, live score, and a real health/food HUD:
  ...
  - **`cleaner`** — wield a cleaning tool (`T` cycles **scraper** / **brush** / **siphon** — the siphon reuses the water-change nozzle); **hold to scrub the per-type algae** off rocks + wood, **siphon the waste** (it dilutes the live chemistry), and **clean the tank** before the 90-second clock — your score is the tank's clean-percent.
