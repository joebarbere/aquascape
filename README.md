...
- [ ] **Algae simulation** — the per-type algae growth model (green-spot / hair / black-beard / diatom) tied to light, nutrients, flow + grazer cleanup is the remaining Stage-13 piece (**Stage 13 F13.6**). The catalog data + the `domain/water-sim` `algae` profiles exist; F13.6 wires growth onto surfaces + the grazing rules. (The test-kit colour-chart readout + undoable water changes shipped in F13.5b — see the feature bullets.) See [`plans/stage-13-aquarium-husbandry.md`](plans/stage-13-aquarium-husbandry.md).
