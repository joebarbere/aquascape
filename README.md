...
- **Feeding** — typed food (flakes float then sink, pellets drop fast, wafers settle, live food darts) plus a quick "Feed tank" scatter; surface / midwater / substrate feeders find food at their band, otos graze algae off rocks (it regrows), detritivores wander the floor
- **Health & hunger** — every fish carries a health (hearts) and hunger meter: starvation and (once Stage 13's chemistry tick lands) bad water erode health, clean water + food restore it. Surfaced live in simulation mode as a **vitality HUD** (school avg / min health · % hungry) with **click-to-inspect** a fish (its hearts + hunger) — not per-fish floating bars
...
- [ ] **Waste → chemistry live loop** — fish vitality & feeding shipped (typed food + animated drops, fish health + hunger, the vitality HUD + click-to-inspect — see the feature bullets above). What remains is closing the loop: the feed → waste → ammonia source term is **produced** (`world.getWasteSourceN()`), but nothing **consumes** it yet — the live `WaterChemistryService` tick that reads it and decays health on bad water is **Stage 13 F13.3 (pending)**. See [`plans/stage-14-fish-vitality-feeding.md`](plans/stage-14-fish-vitality-feeding.md).
- [ ] **Simulation action HUD** — a bottom-center tool HUD: feeding (place the drop) + a multi-step water change (place/move a siphon, siphon out → in); builds the reusable `SiphonTool`. See [`plans/stage-15-husbandry-interactions.md`](plans/stage-15-husbandry-interactions.md).
- [ ] **Game modes** — `--mode game:<submode>` fish-eye, player-controlled. **`predator` is playable** (hunt prey on a timer); `survival` / `feeding` / `cleaner` boot the playable loop but their win/lose rules are gated on Stage 14 (food + health) / Stage 13 (algae) / Stage 15 (`SiphonTool`). ⚑ [ADR-0007](docs/decisions/0007-game-mode-cli-grammar.md). See [`plans/stage-16-game-modes.md`](plans/stage-16-game-modes.md).
