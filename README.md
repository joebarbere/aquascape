...
- [ ] **Water testing UI + water changes** — the live nitrogen cycle now runs end-to-end (see the feature bullets), but the classic test-kit colour-chart readout + the undo-able `WaterChange` Command are still pending (**Stage 13 F13.5**). The editor surfaces a minimal cycle badge + nitrogen readout for now; the full panel is F13.5. See [`plans/stage-13-aquarium-husbandry.md`](plans/stage-13-aquarium-husbandry.md).
