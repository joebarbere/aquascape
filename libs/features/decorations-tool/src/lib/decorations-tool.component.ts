// Decorations palette side panel — classic aquarium ornaments (treasure
// chest, sunken galleon, castle…) from the `core` decor catalog.
//
// Mirrors `HardscapeToolComponent` precisely (the contract for palette
// panels): filterable browser of `core` decor entries with pointer-events
// drag-out to the canvas. Each tile renders its silhouette as an inline
// SVG so the thumbnail looks like the 2D-rendered shape.
//
// Filter UI: a row of category radios (All / Wreck / Ruin / Bones /
// Structure) matching `DecorEntry.category`.
//
// Drag mechanism:
//  - pointerdown on a tile starts the drag via `DecorDragService.start`.
...
//
// Collapsible header: the panel has a self-collapsing header bar at the top
// of the template. The user toggles via click / Enter / Space (native
// <button> semantics). Collapsed state is persisted via `StorageService` so
// the layout survives reloads. Independent per panel.
//
// Paging: the tile grid is paginated 8-per-page to keep the narrow sidebar
// scannable. The pager hides when the visible list fits on one page.
...
type CategoryFilter = 'all' | 'wreck' | 'ruin' | 'bones' | 'structure';

/** Default page size for the tile grid. 2 cols × 4 rows reads well in a sidebar. */
export const DECORATIONS_TOOL_PAGE_SIZE = 8;

/** StorageService key for the collapsed-state flag. */
export const DECORATIONS_TOOL_COLLAPSED_KEY = 'aquascape.ui.collapsed.decorations-tool';

@Component({
  selector: 'aquascape-decorations-tool',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule],
  template: `
    <section class="decorations-tool" aria-labelledby="decorations-tool-heading">
      <header class="panel-header">
        <button
          type="button"
          class="panel-header__toggle"
          [attr.aria-expanded]="!collapsed()"
          aria-controls="decorations-tool-body"
        ...
        >
          <span
            ...
            >›</span
          >
          <h2 id="decorations-tool-heading" class="panel-header__title">Decorations</h2>
          <span class="panel-header__count" aria-label="entries">{{ totalCount() }}</span>
        </button>
      </header>

      <div id="decorations-tool-body" class="decorations-tool__body" [hidden]="collapsed()">
        <div class="decorations-tool__filters" role="radiogroup" aria-label="Category filter">
        ...
        <div class="decorations-tool__grid">
        ...
        @if (visibleEntries().length === 0) {
          <p class="decorations-tool__empty">No decorations match the filter.</p>
        }

        @if (totalPages() > 1) {
          <nav class="pager" aria-label="Decoration pages">
            <button
              type="button"
              class="pager__btn"
...
export class DecorationsToolComponent {
  private readonly dragService = inject(DecorDragService);
   ...
   */
  readonly pageSize = signal<number>(DECORATIONS_TOOL_PAGE_SIZE);

  private readonly allEntries: ReadonlyArray<DecorEntry> = coreCatalog.byKind('decor');

  readonly totalCount = computed<number>(() => this.allEntries.length);

  readonly visibleEntries = computed<ReadonlyArray<DecorEntry>>(() => {
    const f = this.filter();
  ...
  readonly pageEntries = computed<ReadonlyArray<DecorEntry>>(() => {
  ...
  constructor() {
    ...
    this.storage
      .get<boolean>(DECORATIONS_TOOL_COLLAPSED_KEY)
      .then((stored) => {
    ...
    effect(() => {
      ...
      this.storage.set(DECORATIONS_TOOL_COLLAPSED_KEY, value).catch(() => {
        // Best-effort.
      });
    });
  ...
  /** SVG `points=` attribute for a decor silhouette. */
  svgPoints(entry: DecorEntry): string {
  ...
  onPointerDown(event: PointerEvent, entry: DecorEntry): void {
